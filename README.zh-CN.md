...
真正的洞察是：隐私墙必须立在「AI 能读到什么」这条边界上，而这条边界落在 Heptabase 之外，在你怎么把笔记喂给 AI 的那一层。所以这类工具真正要做的，是 **把机密卡片留在 AI 碰不到的地方，只把其余卡片导出成 AI 可读的 Markdown。** 同步笔记只是简单的那一半。
...
由此导出两个设计原则。**结构排除优于减法排除**：查询碰不到的卡片，比读进来再过滤掉更安全。**最好的通知是不需要通知**：工具设计成让你完全不必担心某张卡会不会外泄。
